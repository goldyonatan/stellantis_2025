...
def main():
    ...
    # --- Save predictions (TEST + shadow VAL) ---
    # TEST predictions (one row per segment)
    preds_test = te[[TRIP_ID_COL, SEG_ID_COL, "split"]].copy()
    preds_test["y_true"] = y_te
    preds_test["base_pred"] = base_pred_te
    preds_test["yhat_R3_none"] = yhat_R3
    preds_test["yhat_R1_fp_solo_gated"] = yhat_R1
    preds_test["yhat_R1_fp_cluster_gated"] = yhat_CL
    preds_test["yhat_R1_fp_ens_gated"] = yhat_ENS
    preds_test["yhat_R2_prior"] = yhat_R2
    # residual contributions (useful for debugging)
    preds_test["res_R1_fp_solo_raw"] = r_R1_raw
    preds_test["res_R1_fp_solo_gated"] = r_R1
    preds_test["res_R1_fp_cluster_raw"] = r_CL_raw
    preds_test["res_R1_fp_cluster_gated"] = r_CL
    preds_test["res_ensemble"] = r_ENS

    preds_test.to_csv(PRED_DIR / "test_predictions.csv", index=False)
    try:
        preds_test.to_parquet(PRED_DIR / "test_predictions.parquet", index=False)
    except Exception:
        pass

    # Shadow-VAL predictions used for the ensemble weight selection (diagnostics only)
    preds_val = va[[TRIP_ID_COL, SEG_ID_COL, "split"]].copy()
    preds_val["y_true"] = y_va
    preds_val["base_pred"] = base_pred_va
    preds_val["yhat_reg_val"] = base_pred_va + r_va_reg
    preds_val["yhat_cluster_val"] = base_pred_va + r_va_cl
    preds_val["yhat_ens_val_bestw"] = base_pred_va + (best_w * r_va_reg + (1 - best_w) * r_va_cl)
    preds_val.to_csv(PRED_DIR / "val_predictions_shadow.csv", index=False)

    # Minimal, serializable metadata snapshot
    meta = {
        "best_base_name": best_base_name,
        "ensemble_weight": float(best_w),
        "R1_fp_solo_gated": {k: v for k, v in best_R1.items()
                             if k in ["kv","deg","family","alpha","count_thr","stab_thr","cap_q","shrink"]},
        "R1_fp_cluster_gated": {k: v for k, v in best_CL.items()
                                if k in ["kv","k","count_thr","stab_thr","cap_q","shrink"]},
    }
    (PRED_DIR / "README.txt").write_text(
        "Files:\n"
        "- test_predictions.csv / .parquet: per-segment TEST predictions for all variants\n"
        "- val_predictions_shadow.csv: VAL diagnostics used for ensemble weight selection\n"
        "- test_predictions_meta.json: model selection snapshot\n"
    )
    (PRED_DIR / "test_predictions_meta.json").write_text(json.dumps(meta, indent=2))
