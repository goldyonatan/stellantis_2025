# # quick_sanity_review_t0_nonbehavioral.py
# import pandas as pd
# import numpy as np

# PATH = r"C:\Users\goldy\OneDrive\Documents\Y-DATA 24-25\Stellantis Project\End-to-End\nonbehavioral_t0_features\t0_nonbehavioral_features.parquet"

# # --- Try to import your analyzer; provide a tiny fallback if unavailable ---
# try:
#     from driver_emb_data_preparation import analyze_scalar_feature
# except Exception:
...
# required_keys = [c for c in ["trip_id", "seg_id", "t0_timestamp", "t0_lat", "t0_lon",
#                              "dest_lat", "dest_lon", "crow_distance_km"] if c in df.columns]
# missing_cols = [c for c in required_keys if c not in df.columns]
# if missing_cols:
#     print("\n[WARN] Missing expected columns:", missing_cols)

# # Unique segment rows
# if all(c in df.columns for c in ["trip_id", "seg_id"]):
#     dup = df.duplicated(subset=["trip_id", "seg_id"]).sum()
#     print(f"\nDuplicate (trip_id, seg_id) rows: {dup}")
...
# # ------------------ Range / logic checks ------------------
# def count_bad(mask, name):
#     n = int(mask.sum())
#     if n:
#         print(f"[CHECK] {name}: {n} issue(s)")
#     return n

# issues = 0

# # Lat/Lon ranges
# if "t0_lat" in df and "t0_lon" in df:
#     issues += count_bad(~df["t0_lat"].between(-90,90) | ~df["t0_lon"].between(-180,180), "t0 lat/lon out of range")
# if "dest_lat" in df and "dest_lon" in df:
#     issues += count_bad(~df["dest_lat"].between(-90,90) | ~df["dest_lon"].between(-180,180), "dest lat/lon out of range")

# # Bearing
# if "dest_bearing_deg" in df:
#     issues += count_bad(~df["dest_bearing_deg"].between(0, 360, inclusive="left"), "bearing not in [0,360)")

# # Distances & durations
# if {"crow_distance_km","osrm_distance_km"} <= set(df.columns):
#     bad = (df["osrm_distance_km"].notna() & df["crow_distance_km"].notna() &
#            (df["osrm_distance_km"] + 1e-6 < df["crow_distance_km"]))
#     issues += count_bad(bad, "OSRM distance < crow-flight distance")

# if "osrm_duration_min" in df:
#     issues += count_bad((df["osrm_duration_min"] < 0), "negative OSRM duration")

# # SoC & temp sanity
# if "t0_soc" in df:
#     issues += count_bad((df["t0_soc"] < 0) | (df["t0_soc"] > 100), "t0_soc outside [0,100]")
# if "t0_outside_temp_c" in df:
#     issues += count_bad((df["t0_outside_temp_c"] < -50) | (df["t0_outside_temp_c"] > 60), "t0_outside_temp_c out of plausible range")

# # Time features
# for col, bounds in [("day_of_week",(0,6)), ("is_weekend",(0,1)), ("month",(1,12)),
#                     ("start_hour_sin",(-1.000001,1.000001)), ("start_hour_cos",(-1.000001,1.000001))]:
#     if col in df:
#         lo, hi = bounds
#         issues += count_bad(~df[col].between(lo, hi), f"{col} out of range {bounds}")

# print(f"\nSanity checks found {issues} issue(s).")

# # ------------------ Focused scalar reviews ------------------
# # Only analyze columns that exist
# # to_review = [c for c in [
# #     "t0_outside_temp_c","t0_elev_m","t0_soc","t0_odo",
# #     "dest_elev_m","crow_distance_km","osrm_distance_km","osrm_duration_min",
# #     "dest_bearing_deg","start_hour_sin","start_hour_cos","day_of_week","is_weekend","month",
# #     "battery_capacity_kWh","battery_health","empty_weight_kg"
# # ] if c in df.columns]

# print("\n=== Scalar Feature Spot-Checks ===")
# for c in df.columns:
#     analyze_scalar_feature(df[c], c)

# print("\nDone.")


# curate_review_t0_nonbehavioral.py
# ------------------------------------------------------------
# Standalone post-extraction script that loads the saved
# non-behavioral t0 features parquet and performs:
#   1) Robust DEM elevation extraction (t0 & dest) — list inputs
#      (matches driver-emb behavior) + adds elevation delta
#   2) Keeps existing prints/reports (shape, dtypes, head,
#      missingness, sanity checks)
#   3) Cleans raw outliers -> NaN (scalar thresholds)
#   4) Cleans sanity-check offenders -> NaN
#   5) Drops fully-NaN columns
#   6) Adds cyclic encodings for month & day-of-week
# ------------------------------------------------------------
